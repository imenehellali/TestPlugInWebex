...
@app.route("/auth")
def authorization_shortcut():
    return redirect("/authorization", code=302)
...
def _bearer():
    ...
    return os.environ.get("WEBEX_USER_TOKEN") or os.environ.get("WEBEX_BEARER") or ""
...
def _emit_transcript_to_targets(targets: list[str], payload: dict, state: str = "connected"):
    ...
    agent_name = payload.get("agent_name") or payload.get("agent") or ""
    ...
    customer_number = payload.get("customer_number") or payload.get("customer_phone") or payload.get("number") or ""
    forward_number = payload.get("forward_number") or ""
...
def _parse_v2_bearer() -> dict | None:
    ...
    for secret in _allowed_secret_keys():
        ...
        if token.startswith(secret):
            admin_tenant = token[len(secret) :].strip()
            if admin_tenant:
                return {"secret": secret, "admin_tenant": admin_tenant}
    return None


@app.route("/api/calls/history")
def api_calls_history():
    # Webhook-fed history store, fallback to Webex API if empty and token exists
    if CALL_HISTORY:
...
def placetel_v2_transcripts():
    ...
    if not caller:
        return jsonify({"error": "caller required"}), 400
    if not summary:
    ...
    parsed_bearer = _parse_v2_bearer()
    if not parsed_bearer:
        return jsonify({"error": "invalid bearer"}), 403
    admin_tenant = (payload.get("admin_tenant") or "").strip() or parsed_bearer["admin_tenant"]
    if payload.get("admin_tenant") and payload.get("admin_tenant") != parsed_bearer["admin_tenant"]:
        return jsonify({"error": "admin_tenant does not match bearer"}), 403
    ...
    return jsonify(
        {"ok": True, "stored": True, "call_id": call_id, "admin_tenant": admin_tenant, "version": "v2"}
    )
